...
# Draw fret lines and numbers (start at fret 1, no fake 0 fret)
for fret in range(1, frets + 1):
    x = first_fret_x + ((fret - 1) * fret_spacing)
    fretboard_html += f'<div class="fret-line" style="left:{x}px;"></div>'
    fretboard_html += f'<div class="fret-number" style="left:{x+32}px;">{fret}</div>'

# Inlay markers
for marker_fret in [3, 5, 7, 9]:
    x = first_fret_x + ((marker_fret - 1) * fret_spacing) + 32
    fretboard_html += f'<div class="inlay" style="left:{x}px; top:150px;"></div>'

x12 = first_fret_x + ((12 - 1) * fret_spacing) + 32
